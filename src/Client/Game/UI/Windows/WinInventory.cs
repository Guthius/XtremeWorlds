...
public class WinInventory
{
    public static void OnDraw()
    {
        ...
        {
            var item = GetInv(GameState.MyIndex, slot);
            ...
            if (Trade.InTrade > 0)
            {
                for (var tradeSlot = 0; tradeSlot < Core.Globals.Variables.MaxInventory; tradeSlot++)
                {
                    ...
                    var tempItem = GetInv(GameState.MyIndex, Data.TradeYourOffer[tradeSlot].Num);

                    if (Item.Instance[tempItem].Type != (byte) ItemCategory.Currency ||
                        Data.TradeYourOffer[tradeSlot].Value == GetInvValue(GameState.MyIndex, slot))
                    {
                        skipItem = true;
                    }
                    else
            ...
            if (!skipItem)
            {
                if (icon > 0 && icon <= GameState.NumItems)
                {
                    ...
                    if (GetInvValue(GameState.MyIndex, slot) > 1)
                    {
                        y = top + 20;

                        var x = left + 1;
                        var amount = GetInvValue(GameState.MyIndex, slot) - amountModifier;
    ...
    public static void OnMouseDown()
    {
        ...
        if (slot >= 0)
        {
            ...
            dragBox.Value = GetInv(GameState.MyIndex, slot);
